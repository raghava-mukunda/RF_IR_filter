...
if __name__ == "__main__":
    print("=" * 80)
    print("10 GHz RECTANGULAR-COAX STEPPED-IMPEDANCE LPF — V4 OPTIMIZED GEOMETRY")
    print("=" * 80)
    ...
    if not OPT_FILE.exists():
        raise FileNotFoundError(
            f"V4 requires the optimizer result: {OPT_FILE}\n"
            f"Run first: {opt.name}"
        )

    # Ideal distributed model using optimized lengths.
    ...
    plt.title("10 GHz rectangular-coax stepped-impedance LPF — starting model")
    plt.grid(True, alpha=0.25)
    plt.legend()
    plt.tight_layout()
    plt.savefig("lpf10_v4_ideal_response.png", dpi=180)
    plt.show()

    # Build the EM model.
    sim_dir = Path("sim_lpf10_v4")
    ...
    print("NEXT: run the openEMS full-wave simulation on this optimized geometry.")
