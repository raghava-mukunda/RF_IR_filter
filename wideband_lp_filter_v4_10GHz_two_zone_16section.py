...
RUN_ID = os.environ.get("WB_RUN_ID", "wideband_lp_filter_v4_10GHz_16section_tapered")
...
N_SECTIONS = int(os.environ.get("WB_NSECTIONS", "16"))
...
RING_SPACING = float(os.environ.get("WB_RING", "3.50"))
SECTION_PITCH = float(os.environ.get("WB_PITCH", "7.00"))
...
print()
print("=" * 100)
print("Wideband LP Filter LEAKY COAX — V4 10-GHz SHARP CUTOFF")
print("=" * 100)
print("V4 redesign: 16-section smooth aperture/depth taper:")
...
def add_aperture(section_index, ring_index, x_center, theta_deg):
    # Smooth 16-section leakage taper.
    # Avoids the abrupt weak->strong discontinuity of V3, which can create
    # passband ripple/reflections. Coupling increases progressively toward
    # the center/rear of the filter so the transition stays sharp while the
    # larger number of leakage sections builds broadband rejection.
    A_TAPER = [
        3.10, 3.15, 3.20, 3.25,
        3.35, 3.45, 3.55, 3.65,
        3.80, 3.95, 4.10, 4.25,
    ...
    if section_index >= len(A_TAPER):
        raise ValueError("N_SECTIONS exceeds supported V4 taper length")

    a_local = A_TAPER[section_index]
    d_local = D_TAPER[section_index]
...
print()
print("NOTE: V4 16-SECTION TAPERED SCREENING MODE")
