...
def build_model(sim_path):
    ...
    mesh.AddLine("z", np.arange(-4.0, 4.01, 0.25))
...
if __name__ == "__main__":
    print("=" * 90)
    print("10 GHz RECTANGULAR-COAX LPF + HW ABSORBERS — V4 FULL-WAVE")
    print("=" * 90)
    ...
    print(f"Frequency range     : {FSTART/1e9:.1f}–{FSTOP/1e9:.1f} GHz")
    print(f"Frequency points    : {NFREQ}")

    sim_dir = Path(__file__).resolve().parent / "sim_lpf10_v4_fullwave"
    ...
    FDTD.Run(str(sim_dir), verbose=3)
    ...
    for fg in [1, 5, 7, 8, 9, 9.5, 10, 11, 12, 15, 20, 30, 40, 50, 60, 70, 80, 100]:
        print(
            f"{fg:6.1f} GHz : "
    ...
    pb = freq <= 8.0e9
    sb20 = (freq >= 20.0e9) & (freq <= 100.0e9)
    sb50 = (freq >= 50.0e9) & (freq <= 100.0e9)
    ...
    print(f"50–100 GHz worst S21     : {np.max(S21_dB[sb50]):.4f} dB")

    # Locate the strongest out-of-band transmission windows.
    for lo, hi in [(10, 20), (20, 40), (40, 60), (60, 80), (80, 100)]:
    ...
    np.savez(
        sim_dir / "sparams_v4_fullwave.npz",
        freq_Hz=freq,
        S11=S11,
        S21=S21,
    )
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(sim_dir / "lpf10_v4_fullwave_sparameters.png", dpi=180)
    plt.show()

    print("\nSaved:")
    print(sim_dir / "sparams_v4_fullwave.npz")
    print(sim_dir / "lpf10_v4_fullwave_sparameters.png")
