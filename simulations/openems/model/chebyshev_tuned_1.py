# -*- coding: utf-8 -*-
"""
V12.1-MESHFIX — 7th-order Chebyshev stepped-impedance filter, EM length tuning.

Purpose
-------
V11.1 established a numerically stable openEMS model, but its physical
response was shifted far below the 9 GHz design edge:

    S21(5 GHz)  ~= -0.26 dB
    S21(9 GHz)  ~= -26.10 dB
    S21(20 GHz) ~= -22.96 dB

This version changes ONLY the physical section lengths, while keeping the
validated MSL-port topology and the V11.1 mesh/solver setup.

The first tuning move is deliberately simple:
    V12 lengths = [1.00, 1.25, 2.00, 1.25, 2.00, 1.25, 1.00] mm

These are snapped to the 0.25 mm grid and preserve 7th-order symmetry.
Widths remain:
    50 ohm : 3.50 mm
    HIGH   : 0.50 mm
    LOW    : 12.25 mm

This is an EM tuning experiment, not a claim that these dimensions are
already optimal.  The simulation result decides the next iteration.
"""
...
if abs(FILTER_LENGTH - 10.0) > 1e-12:
    raise RuntimeError("V12.1 section lengths must sum to exactly 10 mm.")
...
print()
print("=" * 82)
print("V12.1 — 7TH-ORDER CHEBYSHEV FILTER / LENGTH TUNING")
print("=" * 82)
...
CSX_FILE = SIM_PATH / "chebyshev_v12_meshfix.xml"
...
csv_path = RESULTS_PATH / "chebyshev_v12.csv"
...
ax.set_title("V12.1 — 7th-Order Chebyshev Stepped-Impedance Filter")
...
sparam_plot = PLOTS_PATH / "chebyshev_v12_sparams.png"
...
ax.set_title("V12.1 — Input Impedance")
ax.set_xlim(ANALYSIS_START / 1e9, ANALYSIS_STOP / 1e9)
ax.grid(True)
ax.legend()
...
zin_plot = PLOTS_PATH / "chebyshev_v12_zin.png"
...
ax.set_title("V12.1 — S-Parameter Power Check")
ax.set_xlim(ANALYSIS_START / 1e9, ANALYSIS_STOP / 1e9)
ax.grid(True)
ax.legend()
...
power_plot = PLOTS_PATH / "chebyshev_v12_power.png"
...
summary_path = RESULTS_PATH / "chebyshev_v12_summary.txt"

summary_text = f"""
V12.1 — 7th-order Chebyshev stepped-impedance EM tuning
