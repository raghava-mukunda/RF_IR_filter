...
assert abs(FILTER_LENGTH / GRID - round(FILTER_LENGTH / GRID)) < 1e-9
...
print()
print("=" * 82)
print("V18 — CLEAN STEPPED-IMPEDANCE FILTER / 100 GHz SWEEP")
print("=" * 82)
...
plt.title("V18 Stepped-Impedance Filter — 1–100 GHz")
...
with GEOM_PATH.open("w", encoding="utf-8") as fh:
    fh.write("V18 PHYSICAL GEOMETRY RECORD\n")
    fh.write("=" * 72 + "\n")
...
if _CANDIDATE_ENV:
    _metrics = {
        ...
        "max_s21_20_100_dB": float(np.max(
            S21_dB[(freq >= 20e9) & (freq <= 100e9)]
        )),
    ...
    # Write the machine-readable metrics used by the optimizer controller.
    # Flush and verify the file before announcing completion so the controller
    # can never receive a path for a file that was not actually created.
    ...
    if not _metrics_path.is_file():
        raise RuntimeError(
            f"Optimizer metrics write failed; file does not exist: {_metrics_path}"
        )

    print(f"OPTIMIZER_METRICS={_metrics_path}")
    print(f"METRICS_EXISTS=True")
