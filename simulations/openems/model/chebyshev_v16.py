# -*- coding: utf-8 -*-
"""V17 — 7th-order stepped-impedance low-pass filter.

Goal:
    Clean cutoff around 9–10 GHz
    Stopband attenuation >=40 dB from 20 GHz onward

V17 is an interpolation between the previous geometries.  V12 had the
strong high/low impedance contrast needed for rejection but its sections
were too electrically long. V17 shortened them too aggressively and lost
stopband rejection. V17 keeps the strong impedance contrast and uses a
moderately shortened, grid-aligned set of section lengths.
...
Section lengths:
    [1.25, 1.50, 2.50, 1.50, 2.50, 1.50, 1.25] mm

Total filter length = 12.00 mm.

The validated MSL/PML/mesh topology is unchanged.
"""
...
SECTION_WIDTHS = [
    W_HIGH, W_LOW, W_HIGH, W_LOW, W_HIGH, W_LOW, W_HIGH,
]
...
if abs(FILTER_LENGTH - 12.00) > 1e-12:
    raise RuntimeError("V17 section lengths must sum to exactly 12.00 mm.")

if not (W_HIGH < W50 < W_LOW):
    raise RuntimeError(
        "V17 width ordering is invalid: expected W_HIGH < W50 < W_LOW."
    )
...
# The V17 filter is centered at x=0.  Its total length is 8.25 mm,
...
# SECTION_LENGTHS are specified in mm, while GRID_PITCH is stored in SI
# metres. Convert the grid pitch to mm before checking alignment.
# Section lengths are specified in millimetres.  Keep this check in
# millimetres; GRID_PITCH is the solver/grid value and must not be blindly
# converted here because this model mixes physical-mm geometry with the
# openEMS unit scale.
SECTION_GRID_MM = 0.25

if any(
    abs(L / SECTION_GRID_MM - round(L / SECTION_GRID_MM)) > 1e-8
    for L in SECTION_LENGTHS
):
    raise RuntimeError(
        "A section length is not aligned to the 0.25-mm grid. "
        f"Lengths={SECTION_LENGTHS}, grid={SECTION_GRID_MM:.6f} mm"
    )
...
print()
print("=" * 82)
print("V17 — 7TH-ORDER CHEBYSHEV FILTER / BALANCED CUTOFF / STOPBAND TUNING")
print("=" * 82)
...
print()
print("V17 PHYSICAL BALANCED CUTOFF / STOPBAND RE-SYNTHESIS")
print("-" * 82)
...
# =====================================================================
#
# Save a human-readable record of the exact V17 physical geometry so that
# the eventual frozen design can be reconstructed without ambiguity.
# =====================================================================

geometry_summary = RESULTS_PATH / "chebyshev_v17_fixed_geometry.txt"

with geometry_summary.open("w", encoding="utf-8") as f:
    f.write("V17 — FINAL CANDIDATE GEOMETRY RECORD\n")
    f.write("=" * 72 + "\n")
...
CSX_FILE = SIM_PATH / "chebyshev_v17_fixed.xml"
...
# V17 is the stopband-tuning candidate before LC extraction.
...
csv_path = RESULTS_PATH / "chebyshev_v17_fixed.csv"
...
ax.set_title("V17 — 7th-Order Chebyshev Stepped-Impedance Filter")
...
sparam_plot = PLOTS_PATH / "chebyshev_v17_fixed_sparams.png"
...
ax.set_title("V17 — Input Impedance")
ax.set_xlim(ANALYSIS_START / 1e9, ANALYSIS_STOP / 1e9)
ax.grid(True)
ax.legend()
...
zin_plot = PLOTS_PATH / "chebyshev_v17_fixed_zin.png"
...
ax.set_title("V17 — S-Parameter Power Check")
ax.set_xlim(ANALYSIS_START / 1e9, ANALYSIS_STOP / 1e9)
ax.grid(True)
ax.legend()
...
power_plot = PLOTS_PATH / "chebyshev_v17_fixed_power.png"
...
summary_path = RESULTS_PATH / "chebyshev_v17_fixed_summary.txt"

summary_text = f"""
V17 — 7th-order Chebyshev stepped-impedance EM tuning
