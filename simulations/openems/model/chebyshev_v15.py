# -*- coding: utf-8 -*-
"""V15 — 9th-order stepped-impedance low-pass filter for a clean 9–10 GHz cutoff
and >=40 dB stopband attenuation.

Target:
    50 ohm system
    passband through approximately 9–10 GHz
    stopband attenuation >=40 dB from 20 GHz onward

V15 uses a symmetric 9-section stepped-impedance network. The higher order
provides additional stopband rejection while the physical section lengths
are kept short enough to push distributed spurious responses upward.
...
Section lengths:
    [1.50, 1.50, 2.50, 1.75, 2.75, 1.75, 2.50, 1.50, 1.50] mm

Total filter length = 17.25 mm.
"""
...
SECTION_WIDTHS = [
    W_HIGH, W_LOW, W_HIGH, W_LOW, W_HIGH,
    W_LOW, W_HIGH, W_LOW, W_HIGH,
]
...
if abs(FILTER_LENGTH - 17.25) > 1e-12:
    raise RuntimeError("V15 section lengths must sum to exactly 17.25 mm.")

if not (W_HIGH < W50 < W_LOW):
    raise RuntimeError(
        "V15 width ordering is invalid: expected W_HIGH < W50 < W_LOW."
    )
...
# The V15 filter is centered at x=0.  Its total length is 8.25 mm,
...
print()
print("=" * 82)
print("V15 — 9TH-ORDER CHEBYSHEV FILTER / STOPBAND TUNING")
print("=" * 82)
...
print()
print("V15 PHYSICAL STOPBAND RE-SYNTHESIS")
print("-" * 82)
...
# =====================================================================
#
# Save a human-readable record of the exact V15 physical geometry so that
# the eventual frozen design can be reconstructed without ambiguity.
# =====================================================================

geometry_summary = RESULTS_PATH / "chebyshev_v15_geometry.txt"

with geometry_summary.open("w", encoding="utf-8") as f:
    f.write("V15 — FINAL CANDIDATE GEOMETRY RECORD\n")
    f.write("=" * 72 + "\n")
    ...
    f.write("\nNOTE: widths are physical CAD/EM dimensions; the historical high/low-impedance\n")
    f.write("labels are not assumed to be exact characteristic impedances.\n")

CSX_FILE = SIM_PATH / "chebyshev_v15.xml"
...
# V15 is the stopband-tuning candidate before LC extraction.
...
csv_path = RESULTS_PATH / "chebyshev_v15.csv"
...
ax.set_title("V15 — 7th-Order Chebyshev Stepped-Impedance Filter")
...
sparam_plot = PLOTS_PATH / "chebyshev_v15_sparams.png"
...
ax.set_title("V15 — Input Impedance")
ax.set_xlim(ANALYSIS_START / 1e9, ANALYSIS_STOP / 1e9)
ax.grid(True)
ax.legend()
...
zin_plot = PLOTS_PATH / "chebyshev_v15_zin.png"
...
ax.set_title("V15 — S-Parameter Power Check")
ax.set_xlim(ANALYSIS_START / 1e9, ANALYSIS_STOP / 1e9)
ax.grid(True)
ax.legend()
...
power_plot = PLOTS_PATH / "chebyshev_v15_power.png"
...
summary_path = RESULTS_PATH / "chebyshev_v15_summary.txt"

summary_text = f"""
V15 — 9th-order Chebyshev stepped-impedance EM tuning
