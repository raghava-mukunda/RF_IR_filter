...
SIM_PATH = PROJECT_ROOT / "simulations" / "openems" / "results" / "two_lpf_cascade_v1"
RESULTS_PATH = PROJECT_ROOT / "results" / "em" / "two_lpf_cascade_v1"
...
# ============================================================================
# OPTIMIZER CANDIDATE OVERRIDES
# ============================================================================

import json as _optimizer_json

_CANDIDATE_ENV = os.environ.get("V18_CANDIDATE_JSON", "")
_OPTIMIZER_MODE = os.environ.get("V18_OPTIMIZER_MODE", "")

if _CANDIDATE_ENV:
    _candidate = _optimizer_json.loads(_CANDIDATE_ENV)

    W_HIGH = float(_candidate["w_high"])
    W_LOW = float(_candidate["w_low"])

    _l1 = float(_candidate["l1"])
    _l2 = float(_candidate["l2"])
    _l3 = float(_candidate["l3"])
    _l4 = float(_candidate["l4"])
    ...
    SECTION_LENGTHS = [
        _l1, _l2, _l3, _l4, _l3, _l2, _l1
    ]

    # Candidate optimization is focused on the region that determines the
    # observed cutoff and the ~15–20 GHz unwanted passband.
    if _OPTIMIZER_MODE == "coarse":
        ANALYSIS_START = 1.0e9
    ...
    # A candidate run uses its own result directory. This keeps the actual
    # EM result and diagnostic files traceable.
    _candidate_tag = str(_candidate.get("tag", "candidate"))
    _candidate_tag = "".join(
        c if c.isalnum() or c in "-_" else "_"
        for c in _candidate_tag
    )

    SIM_PATH = (
        PROJECT_ROOT
        / "simulations" / "openems" / "results"
        / "optimizer" / _candidate_tag
    )
...
# Two identical LPFs are cascaded with a 5-mm 50-ohm interconnect.
# Each LPF is the optimized final geometry from the previous campaign.
CASCADE_GAP = 5.00
CASCADE_LENGTH = 2.0 * FILTER_LENGTH + CASCADE_GAP

LPF1_X0 = -CASCADE_LENGTH / 2.0
LPF1_X1 = LPF1_X0 + FILTER_LENGTH
LPF2_X0 = LPF1_X1 + CASCADE_GAP
LPF2_X1 = LPF2_X0 + FILTER_LENGTH

# Sanity: the two-filter structure is centered.
...
print()
print("=" * 82)
print("TWO IDENTICAL V18 LPFs IN CASCADE — 1–100 GHz SWEEP")
print("=" * 82)
...
print(f"Single LPF length      : {FILTER_LENGTH:.2f} mm")
...
def build_lpf(x_start: float, label: str) -> float:
    """Build one complete 7-section V18 stepped-impedance LPF."""
    x = x_start
...
end1 = build_lpf(LPF1_X0, "LPF1")
if abs(end1 - LPF1_X1) > 1e-12:
    raise RuntimeError(f"LPF #1 does not close: {end1} vs {LPF1_X1}")

# Explicit 50-ohm interconnect between the two filters.
geometry_elements.append(
    (LPF1_X1, LPF2_X0, W50, "INTERCONNECT_50OHM")
)

end2 = build_lpf(LPF2_X0, "LPF2")
if abs(end2 - LPF2_X1) > 1e-12:
    raise RuntimeError(f"LPF #2 does not close: {end2} vs {LPF2_X1}")
...
XML_PATH = SIM_PATH / "two_lpf_cascade_v1.xml"
...
CSV_PATH = RESULTS_PATH / "two_lpf_cascade_v1.csv"
...
SUMMARY_PATH = RESULTS_PATH / "two_lpf_cascade_v1_summary.txt"

with SUMMARY_PATH.open("w", encoding="utf-8") as fh:
    fh.write("TWO-LPF CASCADE FULL-WAVE SUMMARY\n")
    fh.write("=" * 72 + "\n")
    ...
    fh.write(f"single LPF length  : {FILTER_LENGTH:.6f} mm\n")
...
SPLOT = PLOTS_PATH / "two_lpf_cascade_v1_sparams_100GHz.png"
...
plt.title("Two Identical V18 LPFs in Cascade — 1–100 GHz")
...
ZPLOT = PLOTS_PATH / "two_lpf_cascade_v1_zin.png"
...
PLOT_POWER = PLOTS_PATH / "two_lpf_cascade_v1_power.png"
...
GEOM_PATH = RESULTS_PATH / "two_lpf_cascade_v1_geometry.txt"

with GEOM_PATH.open("w", encoding="utf-8") as fh:
    fh.write("TWO-LPF CASCADE PHYSICAL GEOMETRY RECORD\n")
    fh.write("=" * 72 + "\n")
    ...
    fh.write(f"Single LPF length   : {FILTER_LENGTH}\n")
    ...
    for label, x_start in [("LPF1", LPF1_X0), ("LPF2", LPF2_X0)]:
    ...
    fh.write("Transition widths (HIGH -> LOW): 0.50, 3.50, 6.50, 9.50, 12.50 mm\n")
...
# ============================================================================
# MACHINE-READABLE OPTIMIZER METRICS
# ============================================================================

if _CANDIDATE_ENV:
    _metrics = {
        "tag": _candidate.get("tag", "candidate"),
        "w_high": float(W_HIGH),
        "w_low": float(W_LOW),
        "lengths": [float(v) for v in SECTION_LENGTHS],
        "filter_length_mm": float(FILTER_LENGTH),
        "cascade_length_mm": float(CASCADE_LENGTH),
        ...
        "passband_min_1_8p5_dB": float(np.min(
            S21_dB[(freq >= 1e9) & (freq <= 8.5e9)]
        )),
        "passband_max_1_8p5_dB": float(np.max(
            S21_dB[(freq >= 1e9) & (freq <= 8.5e9)]
        )),
        "max_s21_12_20_dB": float(np.max(
            S21_dB[(freq >= 12e9) & (freq <= 20e9)]
        )),
        "max_s21_20_30_dB": float(np.max(
            S21_dB[(freq >= 20e9) & (freq <= 30e9)]
        )),
        "attenuation_20GHz_dB": float(-S21_dB[i20]),
        "power_min": float(np.min(power)),
        "power_max": float(np.max(power)),
    }

    _metrics_path = RESULTS_PATH / "metrics.json"
    with _metrics_path.open("w", encoding="utf-8") as _fh:
        _optimizer_json.dump(_metrics, _fh, indent=2)

    print(f"OPTIMIZER_METRICS={_metrics_path}")

print()
print("=" * 82)
print("TWO-LPF CASCADE FILES")
print("=" * 82)
