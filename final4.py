...
# Broadband Gaussian pulse: deliberately wider than the 1-70 GHz target band.
# With FC=60 GHz, the excitation duration is shortened enough that the
# 120000-step run satisfies the openEMS 3x-excitation-duration check while
# still covering the complete 1-70 GHz analysis band.
END_CRITERIA = 1e-5
...
def piecewise_grid(boundaries, max_step):
    """
    Mesh each interval independently.

    This is the key fix: optimized section boundaries are never generated by
    cumulative floating-point addition followed by a global uniform mesh.
    """

    boundaries = quantized_unique(
        boundaries,
        quantum=1e-4,
    )
...
def build_model(
    ...
    # ------------------------------------------------------------
    #
    # The longitudinal mesh is generated strictly from physical section
    # boundaries. Do NOT inject cylindrical HW radial coordinates into X:
    # doing so can create tiny floating-point sliver cells. The circular HW
    # cross-sections are resolved in Y/Z by the Cartesian mesh.
    # ------------------------------------------------------------
    ...
    # No HW radial coordinates are injected into X.
    ...
    if min(min_dx, min_dy, min_dz) < 0.02:
        print()
        ...
        raise RuntimeError(
            "Pathological mesh cell detected after coordinate cleanup. "
            "The physical geometry was NOT modified."
        )
