...
# First-order frequency scaling:
# l_new = l_published * (13.5 / 10)
...
if __name__ == "__main__":
    print("=" * 80)
    print("10 GHz RECTANGULAR-COAX STEPPED-IMPEDANCE LPF — V1")
    print("=" * 80)
    print(f"Target cutoff       : {FC/1e9:.3f} GHz")
    print(f"Published cutoff    : {PUBLISHED_FC/1e9:.3f} GHz")
    print(f"Length scale        : {SCALE:.4f}")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("lpf10_v1_ideal_response.png", dpi=180)
    plt.show()

    # Build the EM model.
    sim_dir = Path("sim_lpf10_v1")
    ...
    print("NEXT: run the simulation and inspect S11/S21 before optimization.")
